...
function UI.selected_items(self, action_name, range, offset)
  ...
  if range.given and vim.bo.filetype == list_filetype then
    local items = {}
    for _ = range.first, range.last, 1 do
      table.insert(items, self.collector.items)
    end
    return items
  end
  ...
  return {self.collector.items[index + offset]}
end
...
M.new = function(collector, notifier)
  local tbl = {collector = collector, notifier = notifier, windows = {}}
