...
local InputFilter = {}
InputFilter.__index = InputFilter

function InputFilter.new(source_name, filter_name)
  local key = ("%s_%s"):format(source_name, filter_name)
  local tbl = {
    _line_history = _line_histories[key] or {},
    _current_index = 0,
  }
  local self = setmetatable(tbl, InputFilter)
  _line_histories[key] = self._line_history
  return self
end

function InputFilter.recall_history(self, offset, current_line)
...
function InputFilter.append(self, input_line)
  if input_line == "" then
    return
  end
