...
function M.by_name(name, fields)
  local origin = require("thetto.vendor.misclib.module").find("thetto.handler.pipeline.filter." .. name)
  if not origin then
...
function M.relative_path(item_key, name, fields)
  ...
  return require("thetto.util.filter").by_name(name, extended_fields)
end
