...
function M.ui(raw_opts)
  ...
  return function(consumer_ctx, source, pipeline, callbacks, actions)
    ...
    if opts.has_sidecar == nil then
      opts.has_sidecar = require("thetto2.core.kind").can_preview(kind)
    end

    return require("thetto2.handler.consumer.ui").new(consumer_ctx, source, pipeline, callbacks, actions, opts)
  end
end
