...
function M.action_list_pull_request(items)
  for _, item in ipairs(items) do
    local source = require("thetto2.util.source").by_name("github/pull_request", {
      opts = { owner = item.repo.owner, repo = item.repo.name },
    })
    return require("thetto2").start(source)
  end
...
function M.action_list_milestone(items)
  for _, item in ipairs(items) do
    local source = require("thetto2.util.source").by_name("github/milestone", {
      opts = { owner = item.repo.owner, repo = item.repo.name },
    })
    return require("thetto2").start(source)
  end
...
function M.action_list_release(items)
  for _, item in ipairs(items) do
    local source = require("thetto2.util.source").by_name("github/release", {
      opts = { owner = item.repo.owner, repo = item.repo.name },
    })
    return require("thetto2").start(source)
  end
...
function M.action_list_projects(items)
  for _, item in ipairs(items) do
    local source = require("thetto2.util.source").by_name("github/project", {
      opts = { owner = item.repo.owner, repo = item.repo.name },
    })
    return require("thetto2").start(source)
  end
...
function M.action_list_action_workflows(items)
  for _, item in ipairs(items) do
    local source = require("thetto2.util.source").by_name("github/action/workflow", {
      opts = { owner = item.repo.owner, repo = item.repo.name },
    })
    return require("thetto2").start(source)
  end
